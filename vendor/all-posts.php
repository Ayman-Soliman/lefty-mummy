                                ...
                                foreach ($posts as $post) {
                                    ...
                                    <tr>
                                        <td class="txt-c border-bottom" >
                                            <img class="rad-6" width="100" src="uploads/posts/<?= $post['post_cover']?>" alt="">
                                        </td>
                                        <td class="p-20 border-bottom" ><a href="../post.php?post_id=<?=$post['id']?>&slug=<?=$post['post_slug']?>">
                                            <?= $post['title'];?></a></td>
                                        <td class="p-20 border-bottom" ><?=strip_tags(substr($post['post_sammary'], 0, 100))?>...</td>
                                        <td class="p-20 border-bottom <?= $post['status'] ? 'c-main':'c-red';?>" ><?= $post['status'] ? 'Yes':'No';?></td>
                                        <td class="p-20 border-bottom" ><?=date('jS M, Y',strtotime($post['created_at']))?></td>
                                        <td class="p-20 border-bottom" >
                                            <!-- <span class="bg-orange c-white btn p-5" >Edit</span> -->
                                                <a class="btn  txt-c bg-orange c-white rad-6 p-10" href="edit-post.php?id=<?= $post['id'];?>" type="submit" name="e
                                                dit-post-btn">Edit</a>
                                        </td>
                                        <td class="p-20 border-bottom" >
                                            <!-- <span class="bg-orange c-white btn p-5" >Edit</span> -->
